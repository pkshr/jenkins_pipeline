node {
  if(params.Confirm =="false"){
    echo "WARNING: Nothing done. Please start over and check the Confirm box [Click here to confirm startup/shutdown before clicking Build]"
  }
  else{
    started_instances=[]
    ...
    stage('Change instance state (Current to Target)'){
      ...
      if(instances_to_start){
        ...
        aws_out=sh(script:"aws ec2 start-instances --profile es --instance-ids ${instances_to_start}", returnStdout: true).trim()
        echo aws_out
      }
      if(instances_to_stop){
        echo "CHANGE: Following instances will be stopped ${instances_to_stop}"
        aws_out=sh(script:"aws ec2 stop-instances --profile es --instance-ids ${instances_to_stop}", returnStdout: true).trim()
        echo aws_out
      }
    }
